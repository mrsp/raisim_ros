...
//Simulation Step, 0.001 is the hardware loop of the actual talos
double dt = 0.001;
double freq = 1.0 / dt;
...
int main(int argc, char *argv[])
{
  ...
  ros::init(argc, argv, "talos_raisim_ros_publisher");
  ros::NodeHandle n;
  ros::NodeHandle n_p("~");
  ...
  n_p.param<std::string>("modelname",modelname,"../rsc/talos/talos_full_v2.urdf");
...
-0.25847,  -0.173046, 0.0002, -0.525366, 0, 0, 0.1,
 ...
 0.0, 0.00,  -0.4,  0.8, -0.4, 0;
  ...
  talos->setGeneralizedVelocity(jointNominalVelocity);
  ...
  Eigen::VectorXd q(talos->getGeneralizedCoordinateDim()), dq(talos->getDOF());
  /// Set Desired Frames for the WBC
  string lfoot_frame = "leg_left_6_link";
  string rfoot_frame = "leg_right_6_link";
  string base_frame = "base_link";
  string lhand_frame = "arm_left_7_link";
  string rhand_frame = "arm_right_7_link";
  string head_frame = "head_2_joint";

  double mass = talos->getTotalMass();
  bool LSS, RSS, DS;
  ...
  auto RHandFrameIndex = talos->getFrameIdxByName("arm_right_7_joint");
  auto LHandFrameIndex = talos->getFrameIdxByName("arm_left_7_joint");
  auto HeadFrameIndex = talos->getFrameIdxByName("head_2_joint");
  ...
  // INIT ROS PUBLISHER
  ros::Publisher joint_pub = n.advertise<sensor_msgs::JointState>("/talos_raisim_ros/joint_states", 1000);
  ros::Publisher odom_pub = n.advertise<nav_msgs::Odometry>("/talos_raisim_ros/odom", 1000);
  ros::Publisher wrench_pub_LLeg = n.advertise<geometry_msgs::WrenchStamped>("/talos_raisim_ros/LLeg/force_torque_states", 1000);
  ros::Publisher wrench_pub_RLeg = n.advertise<geometry_msgs::WrenchStamped>("/talos_raisim_ros/RLeg/force_torque_states", 1000);
  ros::Publisher com_pub = n.advertise<nav_msgs::Odometry>("/talos_raisim_ros/CoM", 1000);
  ros::Publisher odom_pub_LLeg = n.advertise<nav_msgs::Odometry>("/talos_raisim_ros/LLeg/odom", 1000);
  ros::Publisher odom_pub_RLeg = n.advertise<nav_msgs::Odometry>("/talos_raisim_ros/RLeg/odom", 1000);
  ros::Publisher odom_pub_LHand = n.advertise<nav_msgs::Odometry>("/talos_raisim_ros/LHand/odom", 1000);
  ros::Publisher odom_pub_RHand = n.advertise<nav_msgs::Odometry>("/talos_raisim_ros/RHand/odom", 1000);
  ros::Publisher odom_pub_Head = n.advertise<nav_msgs::Odometry>("/talos_raisim_ros/Head/odom", 1000);
  ros::Publisher zmp_pub = n.advertise<geometry_msgs::PointStamped>("/talos_raisim_ros/ZMP", 1000);
  ros::Publisher LCOP_pub = n.advertise<geometry_msgs::PointStamped>("/talos_raisim_ros/LLeg/COP", 1000);
  ros::Publisher RCOP_pub = n.advertise<geometry_msgs::PointStamped>("/talos_raisim_ros/RLeg/COP", 1000);

  ros::Publisher left_gait_phase_pub  = n.advertise<std_msgs::String>("/talos_raisim_ros/LLeg/contact_status", 1000);
  ros::Publisher right_gait_phase_pub = n.advertise<std_msgs::String>("/talos_raisim_ros/RLeg/contact_status", 1000);
  ros::Publisher gait_phase_pub = n.advertise<std_msgs::String>("/talos_raisim_ros/gait_phase", 1000);

  ros::Subscriber command_js_sub = n.subscribe("/talos_raisim_ros/command_joint_states", 1000, commandJointStatesCallback);

  ros::Publisher imu_pub_LLeg = n.advertise<sensor_msgs::Imu>("/talos_raisim_ros/LLeg/imu", 1000);
  ros::Publisher imu_pub_RLeg = n.advertise<sensor_msgs::Imu>("/talos_raisim_ros/RLeg/imu", 1000);
  ros::Publisher imu_pub = n.advertise<sensor_msgs::Imu>("/talos_raisim_ros/imu", 1000);
  ...
  while (ros::ok())
  {
    ...
    talos->getFramePosition(RfootFrameIndex, footPosition);
    talos->getFrameOrientation(RfootFrameIndex, footOrientation);
    ...
    talos->getFrameVelocity(RfootFrameIndex, footLinearVelocity);
    ...
    talos->getFrameAngularVelocity(RfootFrameIndex, footAngularVelocity);
    RfootAngularVelocity = Eigen::Vector3d(footAngularVelocity[0], footAngularVelocity[1], footAngularVelocity[2]);




    talos->getFramePosition(LfootFrameIndex, footPosition);
    talos->getFrameOrientation(LfootFrameIndex, footOrientation);
    ...
    talos->getFrameVelocity(LfootFrameIndex, footLinearVelocity);
    ...
    talos->getFrameAngularVelocity(LfootFrameIndex, footAngularVelocity);
    ...
    talos->getFramePosition(RHandFrameIndex, footPosition);
    talos->getFrameOrientation(RHandFrameIndex, footOrientation);
    ...
    talos->getFramePosition(LHandFrameIndex, footPosition);
    talos->getFrameOrientation(LHandFrameIndex, footOrientation);
    ...
    //Get Head Position
    talos->getFramePosition(HeadFrameIndex, footPosition);
    talos->getFrameOrientation(HeadFrameIndex, footOrientation);
    ...
    for (auto &contact : talos->getContacts())
    {
      if (contact.skip())
    ...
    raisim::Vec<3> linear_momentum = talos->getLinearMomentum();
    raisim::Vec<3> center_of_mass = talos->getCOM();
    ...
    if (animation_mode)
    {
      ...
      talos->setPdTarget(jointNominalConfig, jointNominalVelocity);

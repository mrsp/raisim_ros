...
int main(int argc, char *argv[])
{
  ...
  ros::init(argc, argv, "digit_raisim_ros_publisher");
  ros::NodeHandle n;
  ros::NodeHandle n_p("~");
  ...
  n_p.param<std::string>("modelname",modelname,"../rsc/digit/digit.urdf");
  n_p.param<std::string>("activation_key",activation_key,"../rsc/activation.raisim");
  cout<<"Activation Path"<<activation_key;
  ...
  Eigen::VectorXd jointNominalConfig(digit->getGeneralizedCoordinateDim()), jointNominalVelocity(digit->getDOF());
  ...
  // INIT ROS PUBLISHER 
  ros::Publisher joint_pub = n.advertise<sensor_msgs::JointState>("/digit_raisim_ros/joint_states", 1000);
  ros::Publisher odom_pub = n.advertise<nav_msgs::Odometry>("/digit_raisim_ros/odom", 1000);
  ros::Publisher wrench_pub_LLeg = n.advertise<geometry_msgs::WrenchStamped>("/digit_raisim_ros/LLeg/force_torque_states", 1000);
  ros::Publisher wrench_pub_RLeg = n.advertise<geometry_msgs::WrenchStamped>("/digit_raisim_ros/RLeg/force_torque_states", 1000);
  ros::Publisher gait_phase_pub = n.advertise<std_msgs::String>("/digit_raisim_ros/gait_phase", 1000);
  ros::Publisher com_pub = n.advertise<nav_msgs::Odometry>("/digit_raisim_ros/CoM", 1000);
  ros::Publisher odom_pub_LLeg = n.advertise<nav_msgs::Odometry>("/digit_raisim_ros/LLeg/odom", 1000);
  ros::Publisher odom_pub_RLeg = n.advertise<nav_msgs::Odometry>("/digit_raisim_ros/RLeg/odom", 1000);
  ros::Publisher odom_pub_LHand = n.advertise<nav_msgs::Odometry>("/digit_raisim_ros/LHand/odom", 1000);
  ros::Publisher odom_pub_RHand = n.advertise<nav_msgs::Odometry>("/digit_raisim_ros/RHand/odom", 1000);
  ros::Publisher odom_pub_Head = n.advertise<nav_msgs::Odometry>("/digit_raisim_ros/Head/odom", 1000);
  ros::Publisher zmp_pub = n.advertise<geometry_msgs::PointStamped>("/digit_raisim_ros/ZMP", 1000);
  ros::Publisher LCOP_pub = n.advertise<geometry_msgs::PointStamped>("/digit_raisim_ros/LLeg/COP", 1000);
  ros::Publisher RCOP_pub = n.advertise<geometry_msgs::PointStamped>("/digit_raisim_ros/RLeg/COP", 1000);
  ros::Subscriber command_js_sub = n.subscribe("/digit_raisim_ros/command_joint_states", 1000, commandJointStatesCallback);
  
  ros::Publisher imu_pub_LLeg = n.advertise<sensor_msgs::Imu>("/digit_raisim_ros/LLeg/imu", 1000);
  ros::Publisher imu_pub_RLeg = n.advertise<sensor_msgs::Imu>("/digit_raisim_ros/RLeg/imu", 1000);
  ros::Publisher imu_pub = n.advertise<sensor_msgs::Imu>("/digit_raisim_ros/imu", 1000);
  ...
  while (ros::ok())
  {
    ...
    digit->getFramePosition(RfootFrameIndex, footPosition);
    digit->getFrameOrientation(RfootFrameIndex, footOrientation);
    ...
    digit->getFrameVelocity(RfootFrameIndex, footLinearVelocity);
    ...
    digit->getFrameAngularVelocity(RfootFrameIndex, footAngularVelocity);
    RfootAngularVelocity = Eigen::Vector3d(footAngularVelocity[0], footAngularVelocity[1], footAngularVelocity[2]);




    digit->getFramePosition(LfootFrameIndex, footPosition);
    digit->getFrameOrientation(LfootFrameIndex, footOrientation);
    ...
    digit->getFrameVelocity(LfootFrameIndex, footLinearVelocity);
    ...
    digit->getFrameAngularVelocity(LfootFrameIndex, footAngularVelocity);
    ...
    digit->getFramePosition(RHandFrameIndex, footPosition);
    digit->getFrameOrientation(RHandFrameIndex, footOrientation);
    ...
    digit->getFramePosition(LHandFrameIndex, footPosition);
    digit->getFrameOrientation(LHandFrameIndex, footOrientation);
    ...
    //Get Head Position
    digit->getFramePosition(HeadFrameIndex, footPosition);
    digit->getFrameOrientation(HeadFrameIndex, footOrientation);
    ...
    for (auto &contact : digit->getContacts())
    {
      if (contact.skip())
    ...
    raisim::Vec<3> linear_momentum = digit->getLinearMomentum();
    raisim::Vec<3> center_of_mass = digit->getCOM();

...
Eigen::VectorXd jointNominalConfig, jointVelocityTarget;
...
void commandJointStatesCallback(const sensor_msgs::JointState::ConstPtr& msg)
{
  ...
  if(msg->position.size() > 0)
  {
    ...
    jointNominalConfig.tail(jointNominalConfig.size() - 7) = Eigen::Map<Eigen::Matrix<double, 26, 1> >(pos_array);

    if(msg->velocity.size()>0)
    {
      std::vector<double> vel_vector = msg->velocity;
      double* vel_array = vel_vector.data();
      jointVelocityTarget.tail(jointVelocityTarget.size() - 6) = Eigen::Map<Eigen::Matrix<double, 26, 1> >(vel_array);
    }
    else
    {
...
int main(int argc, char *argv[])
{
  ...
  jointVelocityTarget.resize(NAO->getDOF());
  ...
  // std::vector<double> q_vec(q_size,0), dq_vec(dq_size,0);
// INIT ROS PUBLISHER
  ...
  while (ros::ok())
  {
    ...
    auto start = high_resolution_clock::now();
      ...
      // cout<<"Center Of Mass "<<CoM_pos.transpose()<<endl;



    NAO->setPdTarget(jointNominalConfig, jointVelocityTarget);
...
///////////////////////////////// PUBLISHING ///////////////////
    ...
    odom_msg.pose.pose.orientation.y = q(4);
    odom_msg.pose.pose.orientation.z = q(5);
    odom_msg.pose.pose.orientation.w = q(6);
